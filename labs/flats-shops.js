/* =====================================================================
   THE FLATS SHOPS -- hood-specific copies (Sir, 2026-09-17).
   =====================================================================
   "we have to do the duplicated shops and go in and individually edit
   them." Each entry here STARTED as a verbatim copy of its `base` in
   shopfront-shops.js and is now its own building: renamed for The Flats
   and edited by hand, one shop at a time, in flats-shop-lab.html.

   THESE ARE FORKS, ON PURPOSE. A fix to a base body in
   shopfront-shops.js does NOT reach its Flats copy. When a base gets a
   geometry fix that its copy still carries (the census patterns: face
   circles, windows over doors, flat panels, stacking order), port it
   here by hand. `base` is kept on every entry so that sweep is a grep.

   `sc` IS THE SHOP'S DRAWN SCALE (Sir, 2026-09-17). Every single, double
   and triple lot shop carries sc:1.3 -- they read too small against the
   blocks. The body is still written in its own units (W 230, D 276, the
   real door) and the host scales it on the ground: its footprint is
   (ww || W) x (dd || D) times sc, which is what a packer must reserve.
   Full-edge (1048.8) and block-landmark buildings carry none and stay 1x.
   Honour it in P(): a, b and z all multiply by sc, after any turn.

   `edited` says which ones have had their pass. An entry with
   edited:false is still byte-for-byte its base body under a new name.

   Same host contract as shopfront-shops.js (see its header): same kit,
   same frame, same benches. The game grafts these into LIB and places
   them by name from HOOD_SHOP_SITES.
   ===================================================================== */


/* ---- HELPERS, forked with the shops that call them (flats-prefixed so this
   file and shopfront-shops.js can load into one scope without a clash) ---- */
const FLATS_CHEMIST_LIVERY = {
...
/* the Flats liveries, added alongside the forked base ones */
FLATS_CHEMIST_LIVERY.seafoam = { trim:'#3f9e8a', inner:'#8fbcb2', glass:'rgba(150,205,200,.55)', cross:'#3fae9f' };
FLATS_CHEMIST_LIVERY.coral   = { trim:'#d9735a', inner:'#c9a498', glass:'rgba(215,170,160,.55)', cross:'#e2574c' };
const FLATS_CR = { len:225, wid:90, chassisH:42, cabinH:48, wheelR:24,
...
function flatsDrawChemist(p, c){
    const wall = '#f0f2f0', trim = c.trim, H = 172;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#b3bcb8');   // Flats roof, over body's trim-coloured plate (both liveries)
...
function flatsGameCar(cP, col){
    const zk = 1/ZSCALE;
    const hl = FLATS_CR.len/2, hw = FLATS_CR.wid/2, cz = FLATS_CR.wheelR*zk;
    const chassisTop = (FLATS_CR.wheelR + FLATS_CR.chassisH)*zk;
    const cabinTop   = (FLATS_CR.wheelR + FLATS_CR.chassisH + FLATS_CR.cabinH)*zk;
    ...
    const wheel = (a0, r, side) => {
      ...
      poly(ring(bIn, r), FLATS_CR.wheelDk);
      const faceB = cd(a0,bOut,cz) > cd(a0,bIn,cz) ? bOut : bIn;
      poly(ring(faceB, r), FLATS_CR.wheel);
      poly(ring(faceB, r*0.5), FLATS_CR.hubFace);
      const h = cP(a0, faceB, cz);
      ctx.beginPath(); ctx.arc(h.x, h.y, r*0.30*K, 0, 7); ctx.fillStyle = FLATS_CR.hub; ctx.fill();
    };
    const chassis = () => {
    ...
    const cabin = () => {
      ...
      poly([cP(roofR,-hw*0.85,cabinTop),cP(roofR,hw*0.85,cabinTop),
            cP(cl, hw*0.90,chassisTop),cP(cl,-hw*0.90,chassisTop)], FLATS_CR.windshield, FLATS_CR.windshieldEdge, 1);
      poly([cP(roofF,-hw*0.86,cabinTop),cP(roofF,hw*0.86,cabinTop),
            cP(-cl, hw*0.92,chassisTop),cP(-cl,-hw*0.92,chassisTop)], FLATS_CR.windshield, FLATS_CR.windshieldEdge, 1);
      ...
      poly(P4.map(q => cP(c[0]+(q[0]-c[0])*0.7, c[1]+(q[1]-c[1])*0.7, c[2]+(q[2]-c[2])*0.7)), FLATS_CR.windshield);
    };
    const bumper = () => {
      const front = cd(hl,0,cz) > cd(-hl,0,cz), e = front ? hl : -hl, i = front ? hl-0.3 : -hl+0.3;
      poly([cP(i,-hw,cz),cP(e,-hw,cz+3*zk),cP(e,hw,cz+3*zk),cP(i,hw,cz)], FLATS_CR.bumper);
      for(const sgn of [-1,1]){
        const lp = cP(e - (front?0.4:-0.4), sgn*(hw-3), cz+5*zk);
        ctx.beginPath(); ctx.arc(lp.x, lp.y, 2*K, 0, 7);
        ctx.fillStyle = front ? FLATS_CR.light : FLATS_CR.tail; ctx.fill();
      }
    };
    ...
    wheel(-hl*0.55, FLATS_CR.wheelR,      -rearNear);
    wheel( hl*0.55, FLATS_CR.wheelR*0.95, -frontNear);
    chassis();
    if(cd(-hl*0.55, hw*rearNear,  cz) <= cabRef) wheel(-hl*0.55, FLATS_CR.wheelR,      rearNear);
    if(cd( hl*0.55, hw*frontNear, cz) <= cabRef) wheel( hl*0.55, FLATS_CR.wheelR*0.95, frontNear);
    cabin();
    bumper();
    if(cd(-hl*0.55, hw*rearNear,  cz) >  cabRef) wheel(-hl*0.55, FLATS_CR.wheelR,      rearNear);
    if(cd( hl*0.55, hw*frontNear, cz) >  cabRef) wheel( hl*0.55, FLATS_CR.wheelR*0.95, frontNear);
  }
function flatsWallFrames(WW, DD, OA, OB){
...
const SHOPS = [
{
  ...
  draw(p){
    ...
    /* ============ THE FLATS PASS (2026-09-17) ============
       Lagoon Coffee Roasters. The base is a dark-brown city roastery;
       The Flats is the bright, tidy coastal hood, so the building goes
       sun-bleached plaster with lagoon-teal trim and a pale roof, and the
       drum -- teal in the base, which would vanish against teal trim --
       goes coral enamel so the roaster is still the first thing the eye
       finds in the window. Geometry untouched: every edit is a colour. */
    const wall = '#e7dfcf', trim = '#2f7f86', H = 210;
    const inner = '#16201f', copper = '#b87333';
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#8c9a98');                   // roof, over body's pale plate
    ...
    cyl(58, -18, 34, 82, 22, '#c8553d');
    plateCircle(58, -18, 82, 22, '#d8705a', '#7a3022', 2);
    ...
    ctx.restore();
    glaze(14, 128, 22, 118, null, 'rgba(140,180,185,.30)');
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandpiper Bakery. Cream wall kept; the trim was the lab
       palette's brick red (p[0]) and goes coral, and the bunting
       takes coral, cream and sky instead of the palette's gold
       and teal, so it reads as a beach bakery and sits apart from
       the teal roaster next door.
       Geometry untouched: every edit is a colour. */
    ...
    if(state.props){
      ...
      for(let i=0;i<11;i++){
        ...
        poly([P(a-7,bb,z),P(a+7,bb,z),P(a,bb,z-flag)], ['#7cbad0','#f2e6cc','#d9735a'][i%3]);
      }
    }
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Florist. Wall goes bright white, trim from garden
       green to seafoam, and the glazing tint cools to match.
       Stems keep their green.
       Geometry untouched: every edit is a colour. */
    const wall = '#f6f7f2', trim = '#4f9e8a', H = 156;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#a9b5ae');   // Flats roof, over body's trim-coloured plate
    ...
    shopDoor(180, wall, trim, 'rgba(150,205,200,.5)');
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Creamery. Pink-on-pink goes buttercup wall with sky-
       blue trim and awning stripes: still an ice cream parlour,
       but a seaside one, and not a second coral front beside the
       bakery.
       Geometry untouched: every edit is a colour. */
    const wall = '#fbf1d2', trim = '#4a9fd1', H = 140;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#e3dccb');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    ...
    /* ============ THE FLATS PASS (2026-09-17) ============
       Tidewater Tea House. Lacquer red and red tile go deep navy
       trim and blue-glazed tile -- tidewater, not temple. Gold,
       jade and the lanterns stay as the accents.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    ...
    /* ============ THE FLATS PASS (2026-09-17) ============
       Driftwood Books. Brown wall goes weathered driftwood grey,
       trim goes a cleaner off-white, so the tallest building on
       the row reads coastal rather than city-old.
       Geometry untouched: every edit is a colour. */
    const wall = '#9c9384', trim = '#f3eee2', H = 336, GF = 168;
    const inner = shade(wall,.35);
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#b7b1a6');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    ...
    /* ============ THE FLATS PASS (2026-09-17) ============
       Marina Optical. Already white and navy; the navy goes a
       brighter marina blue so it is not the tea house's navy two
       doors up.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#9aa6ad');   // Flats roof, over body's trim-coloured plate
    slab(0,W, H, H+8, -1, -12, trim);
...
},
{
  ...
  draw(p){
    ...
    /* ============ THE FLATS PASS (2026-09-17) ============
       Palmline Bikes. Dark teal wall with orange trim goes palm
       green with sunshine-yellow trim; the frames on display take
       the trim, so the bikes go yellow with it.
       Geometry untouched: every edit is a colour. */
    const wall = '#3f7a5a', trim = '#f2c14e', H = 210;
    const inner = '#17302a', glass = 'rgba(130,178,170,.42)';
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#2c4a3a');                   // roof, over body's trim-coloured plate
    slab(0, W, H, H+10, -1, -12, trim);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Pelican Photo. Concrete grey and charcoal go white with
       pelican slate-blue trim.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    ...
    /* ============ THE FLATS PASS (2026-09-17) ============
       Saltwater Sweets. Pink wall and red trim go pale aqua with
       coral trim -- saltwater taffy colours. The jars keep their
       sweets.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#8fa8a6');                  // roof, over body's trim-red plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Low Tide Records. Black and red go deep-ocean navy with
       tide-yellow trim; still the dark building on the row, but
       it belongs to the sea.
       Geometry untouched: every edit is a colour. */
    const wall = '#1f2f44', trim = '#f2b134', H = 164;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#34404f');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    ...
    const WW = T2*4.4;
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandcastle Toys. White wall goes warm sand, red trim goes
       ocean blue; the toy primaries on the roof and in the
       windows stay loud.
       Geometry untouched: every edit is a colour. */
    ...
    const inner = '#2a2f36';
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#d8c9a8');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ IT WAS A KILN WORKS, NOT A STUDIO ============
       ...
       the board's own -9 backing. */
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Pottery. The terracotta goes onto the trim and the
       wall goes whitewash, the way a seaside pottery looks; the
       clay pieces keep their own colours.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Pelican Pets. Sand wall kept, garden-green trim goes sea
       blue so the corner of the row is not a second green front
       after the bikes.
       Geometry untouched: every edit is a colour. */
    const wall = '#e0d3b8', trim = '#3c7fa0', H = 154;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#a9b1ab');   // Flats roof, over body's trim-coloured plate
    slab(0,W, H, H+10, -1, -12, trim);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandpiper Suds. Navy-on-grey goes sea-glass: pale aqua
       wall, lagoon trim.
       Geometry untouched: every edit is a colour. */
    const wall = '#e4f1f0', trim = '#2f8a8f', H = 158;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#aab4b2');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Low Tide Barbers. Forest green and gold go navy with a sand
       trim -- a seaside barber, still the dark front on its row.
       Geometry untouched: every edit is a colour. */
    const wall = '#24435a', trim = '#e9d7a8', H = 176;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#36465a');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Grocer. Brick-orange wall goes sand, cream trim
       goes sea green.
       Geometry untouched: every edit is a colour. */
    const wall = '#e8dcc0', trim = '#3f8f6a', H = 150;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#b3ab98');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Lagoon Noodle Bar. Lacquer red wall goes lagoon teal; the
       gold trim stays.
       Geometry untouched: every edit is a colour. */
    const wall = '#2f6f7a', trim = '#f2d98c', H = 160;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#2a4a52');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Breakwater Hardware. Mustard and dark brown go sand with
       harbour-blue trim.
       Geometry untouched: every edit is a colour. */
    const wall = '#d9c9a3', trim = '#2f4a6b', H = 180;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#a39a86');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Surfside Diner. The red trim goes retro aqua; the white
       wall stays.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Picture House. Midnight blue goes deep sea teal;
       gold and cream stay, it is still the marquee.
       Geometry untouched: every edit is a colour. */
    ...
    const glass = '#5a7f96';
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#1c3440');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandpiper Guest House. A rooming house in brick goes a
       guest house in white with blue trim.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#8a9699');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Fish Co.. Already blue and white; the trim moves to a
       brighter sea blue.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#9fb0b5');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Palmline Garage. Gunmetal and orange go stucco white with
       sea-blue trim.
       Geometry untouched: every edit is a colour. */
    ...
    const WW = T2*4.4;
    const wall = '#e6e1d6', trim = '#3c7fa0', H = 158;
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#9aa3a6');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Marina Tailors. Plum goes navy; the gold stays.
       Geometry untouched: every edit is a colour. */
    const wall = '#2c4a6b', trim = '#d8c48a', H = 172, WW = 170;
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#243850');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Pelican Cantina. Ochre and brown go pale adobe with coral
       trim.
       Geometry untouched: every edit is a colour. */
    const wall = '#f0d9b0', trim = '#d9735a', H = 152, pb = 52;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#c9b89a');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Tidewater News. Bottle green goes sea teal; the yellow
       stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#2e5a62');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Palmline Savings. Grey stone goes bright limestone with a
       sea-grey trim.
       Geometry untouched: every edit is a colour. */
    ...
    const WW = T2*4.4;
    const wall = '#f0ece2', trim = '#6f8f96', H = 460;
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#b9bcb6');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    ...
    body(wall, trim, H, KW);
    T(0, KW, -D, 0, H+0.4, '#b9bfbc');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Driftwood Chapel. Stone goes whitewash, brown trim goes sea
       blue.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sunfish Arcade. Black goes deep navy; the neon stays.
       Geometry untouched: every edit is a colour. */
    const wall = '#1f2f44', trim = '#37e0d0', H = 168;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#26364a');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Butchers. Oxblood trim goes deep sea blue.
       Geometry untouched: every edit is a colour. */
    const wall = '#f0ece2', trim = '#2f5f7a', H = 150;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#aab2b4');   // Flats roof, over body's trim-coloured plate
    slab(0,W, H, H+10, -1, -12, trim);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sea Glass Exchange. Aubergine goes sea-glass green; the
       gold stays.
       Geometry untouched: every edit is a colour. */
    const wall = '#35555a', trim = '#c9a24a', H = 176, WW = 178;
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#2c4448');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       The Flats Post Office. Wall brightens; the post-office blue
       stays.
       Geometry untouched: every edit is a colour. */
    const wall = '#eee9dc', trim = '#1f4a6b', H = 186;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#a6aca9');   // Flats roof, over body's trim-coloured plate
    slab(0,W, H, H+10, -1, -12, trim);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Surfside Fitness. Gunmetal and orange go pale concrete with
       coral trim; the interior was a shade of the wall and is
       pinned dark so the glass still reads.
       Geometry untouched: every edit is a colour. */
    ...
    const WW = T2*4.4;
    const wall = '#dfe4e2', trim = '#e2574c', H = 210;
    const inner = '#2f3a3f', gear = '#cfd6db', lit = trim;
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#a9b1af');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Hats. Mauve goes teal-blue; the cream trim stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#2f5566');   // Flats roof, over body's trim-coloured plate
    slab(0,WW, H, H+10, -1, -12, shade(wall,.7));
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Lagoon Baths. Already tiled in sea colours; the trim lifts
       a shade toward lagoon.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Locksmith. Brown-grey goes weathered driftwood; the
       brass stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#6f6b62');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Driftwood Home. Taupe and brown go sand with slate-blue
       trim.
       Geometry untouched: every edit is a colour. */
    ...
    const wall = '#e4dccb', trim = '#4d6a86', H = 336, WW = T2*4.4;   // 404.8
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#b4ad9c');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Palmline Nursery. Wall lightens to sand, green trim goes
       brighter sea green.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Low Tide TV & Radio. Grey and black go sea-grey with navy
       trim.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#7f9095');   // Flats roof, over body's trim-coloured plate
    slab(0,W, H, H+10, -1, -12, trim);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Playhouse. Burgundy goes deep sea blue; the cream
       stays.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       The Flats Fire Station. Fire-station red stays -- that is
       what it is -- but the dark roof goes a light grey to sit
       with the hood.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       The Sandpiper Arms. Pub green goes deep sea blue; the gold
       stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#243a48');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Driftwood Antiques. Plum goes weathered sea grey.
       Geometry untouched: every edit is a colour. */
    ...
    const inner = '#1f2a2e', glass = 'rgba(120,116,132,.44)';
    const BAYS = [[16,170],[186,340],[438,592]];
    body(wall, trim, H, WW, DD);
    T(0, WW, -DD, 0, H+0.4, '#3f555c');                 // roof, over body's near-white plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Tidewater Clocks. Slate goes deep teal; the brass stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#1f3f4a');                  // roof, over body's near-white plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Fabrics. Plum goes coral; the cream stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#b8847a');                   // roof, over body's pale plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Low Tide Music. Racing green goes deep sea teal; the brass
       stays.
       Geometry untouched: every edit is a colour. */
    ...
    const inner = '#132a2b';
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#224a4b');                   // roof, over body's pale plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Marina Chandlery. Already a quay shop; the slate goes a
       bluer navy.
       Geometry untouched: every edit is a colour. */
    ...
    const inner = '#111d24';
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#1f3548');                   // roof, over body's pale plate
    slab(0, W, H, H+10, 4, 0, trim);                    // cornice
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Saltwater Brewing. Brown goes weathered sea grey; copper
       and brass stay.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Tidewater Press. Charcoal goes sea grey.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW, DD);
    T(0, WW, -DD, 0, H+0.4, '#4f656c');                 // roof, over body's pale plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandpiper Shoes. Black trim goes navy, the red accent goes
       coral.
       Geometry untouched: every edit is a colour. */
    ...
    const wall = '#e8e6e2', trim = '#24435a', H = 244, WW = T2*4.4;   // 404.8
    const accent = '#e2574c', inner = '#f2f0ec';
    ...
    body(wall, trim, H, WW);
    T(0, WW, -D, 0, H+0.4, '#2f4a5e');                  // roof, over body's pale plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Palmline Surf Co.. Already aqua and sand -- the one shop
       that was built for this hood. Name only.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Breakwater Ironworks. Soot brown goes sea-weathered grey;
       the forge glow stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW, DD);
    T(0, WW, -DD, 0, H+0.4, '#3f4b4f');                 // roof, over body's pale plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Rugs. Maroon goes teal-blue; the gold stays.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#2f5566');                   // roof, over body's pale plate
    slab(0, W, H, H+10, 4, 0, shade(wall,.7));          // cornice
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Gull Loft. Pigeons at the sea are gulls. Brown goes sand,
       trim goes slate blue.
       Geometry untouched: every edit is a colour. */
    ...
    const inner = '#2a2318';
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#8f887a');                   // roof, over body's pale plate
    slab(0, W, H, H+10, 4, 0, trim);                    // cornice
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Dance. Lilac goes seafoam, trim goes teal.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#93b3b0');                   // roof, over body's pale plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Kelp & Ink. Black goes deep-sea navy, red goes coral.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#1a2c36');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandcastle Models. Blue wall and cream trim swap: sand
       wall, blue trim.
       Geometry untouched: every edit is a colour. */
    /* ================= WHAT WAS WRONG =================
    ...
    const zk = 1/ZSCALE;
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#b3a88f');   // Flats roof, over body's trim-coloured plate
    slab(0,W, H, H+10, -1, -12, trim);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Dune Home & Garden. Grey box with an orange band goes sand
       with a lagoon band.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Pelican Drug. The red band goes sea blue.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Marina Flats. Tan and brown go white stucco with sea-blue
       trim.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Palmline Motors. The blue band goes sea green.
       Geometry untouched: every edit is a colour. */
    /* ============ ONE SHOP, NOT A PARADE ============
       ...
       THE CARS ARE SOLIDS, not decals. qbox() in flatsWallFrames picks which
    ...
    /* the car is flatsGameCar() at file scope now -- the Car park is its
    ...
    const carIn = (fr, uc, nc, vf, col) =>
      flatsGameCar((a,b,h) => Q(fr, uc+a, nc+b, vf+h), col);
    ...
    const showroom = (fr, u0, u1, show, gap) => {
      ...
      /* the livery is named per car rather than derived: the first pass
         ...
         four FLATS_CAR_COLORS being nearly the same colour. */
      for(const [ua, liv] of show) carIn(fr, ua, -58, 20, FLATS_CAR_COLORS[liv]);
      ctx.restore();
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Tidewater & Co.. Grey stone brightens, burgundy trim goes
       navy.
       Geometry untouched: every edit is a colour. */
    /* ============ A DEPARTMENT STORE IS A WHOLE EDGE ============
       ...
       four faces through flatsWallFrames, which is now shared by three shops
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seawall Chambers. Stone brightens, slate trim goes navy.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#8f969a');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       The Grand Pelican. Burgundy trim goes deep sea blue on a
       brighter wall; gold and slate stay.
       Geometry untouched: every edit is a colour. */
    /* ============ A GRAND HOTEL IS A WHOLE EDGE ============
       Block WIDTH -- ww 1048.8, dd 620, on the line, no yard -- elevated
       on all four faces through flatsWallFrames, shared by four shops now and
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Driftwood Lofts. Red brick goes weathered grey.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#6f6a60');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Library. Olive trim goes sea blue.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H);
    T(0, W, -D, 0, H+0.4, '#9da6a4');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Marina Parking. Concrete is concrete. Name only.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Lagoon Market Hall. Green trim goes lagoon teal.
       Geometry untouched: every edit is a colour. */
    ...
    /* ============ A HALL IS A NAVE, so it is long and it is narrow ====
       ...
       Same relationship the School's playground has to the School.
         ...
         hall      a    0.. 460
         yard      a  470..2380 */
    const wall = '#c4bda8', trim = '#2f7f86', H = 360, steps = 14, RISE = 190;
    const WW = 460, DD = 2392, glassT = 'rgba(127,152,160,.86)';
    const DMID = WW/2, S0 = DMID - 37.12, S1 = DMID + 37.12;

    body(wall, trim, H, WW, DD);
    T(0, WW, -DD, 0, H+0.4, '#9fa9a4');   // Flats roof, over body's trim-coloured plate
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       The Tide Gazette. Charcoal goes deep sea blue.
       Geometry untouched: every edit is a colour. */
...
{
  name:'Seabreeze Exchange', xh: 430, base:'Telephone exchange', hood:'The Flats', edited:true, tall:true, block:true, ww: 3128, dd: 3128,
  wTodo:'a FULL block cell -- BLOCK 3128 with ROAD_HALF 368 each side, so 1656 square of buildable ground; the packer places no landmark',
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Exchange. Dun goes a lighter sand.
       Geometry untouched: every edit is a colour. */
    /* ============ TIGHT TO THE PAVEMENT, THE REST FENCED ============
       ...
       frontage is at b -736. So it is drawn through flatsWallFrames with an
    ...
    const wall = '#c9c3b4', trim = '#5c5a50', glassT = 'rgba(106,122,128,.88)';
    ...
    if(state.props){
      ...
      for(const [va, vb] of [[2140,-880],[1320,-2180]])
        items.push({ a:va, b:vb, z:0, draw:() => flatsGameCar((a,b,h) => P(va + b, vb - a, h), FLATS_CAR_COLORS[1]) });
    }
    depthSort(items);
    kerb(p,'none');
  }
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       The Flats Police. Brown stone goes pale limestone; the blue
       stays.
       Geometry untouched: every edit is a colour. */
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Tidewater Maritime Museum. Stone brightens, grey trim goes
       sea blue.
       Geometry untouched: every edit is a colour. */
...
{
  name:'Saltwater Mill', xh: 460, base:'Textile mill', hood:'The Flats', edited:true, tall:true, block:true, ww: 3128, dd: 3128,
  wTodo:'a FULL block cell -- BLOCK 3128 with ROAD_HALF 368 each side, so 1656 square of buildable ground; the packer places no landmark',
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Saltwater Mill. Red brick goes weathered sand.
       Geometry untouched: every edit is a colour. */
    ...
    const wall = '#b9ae9c', trim = '#d8cdb8', glassT = 'rgba(106,127,140,.86)';
    const { FR_FRONT, FR_RIGHT, FR_LEFT, FR_BACK, NEAR, FAR, Q, R, bandF, rev, glz, doorF }
    ...
    if(state.props){
      ...
      items.push({ a:2260, b:-900, z:0, draw:() =>
        flatsGameCar((a,b,h) => P(2260 + b, -900 - a, h), FLATS_CAR_COLORS[3]) });
    }
    depthSort(items);
    kerb(p,'none');
  }
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Seabreeze Ballroom. Violet goes deep sea blue; the gold
       stays.
       Geometry untouched: every edit is a colour. */
    /* ============ A BALLROOM IS ONE ROOM ============
       ...
       All four faces are elevations, so it goes through flatsWallFrames with
    ...
    const wall = '#2c5a75', trim = '#e0c88a';
    const { FR_FRONT, FR_RIGHT, FR_LEFT, FR_BACK, NEAR, FAR, Q, R, bandF, rev, glz, doorF }
      = flatsWallFrames(LW, LD, A0, B1);
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Marina Office. Already harbour colours; a lighter roof to
       sit with the hood.
       Geometry untouched: every edit is a colour. */
    ...
    body(wall, trim, H, WW, DD);
    T(0, WW, -DD, 0, H+0.4, '#9aa6a8');   // Flats roof, over body's trim-coloured plate
    slab(0,WW, H, H+12, -1, -14, trim);
...
{
  name:'Cove Ice House', xh: 420, base:'Cold store', hood:'The Flats', edited:true, tall:true, block:true, ww: 3128, dd: 3128,
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Cove Ice House. Grey trim goes sea blue.
       Geometry untouched: every edit is a colour. */
    ...
    if(state.props){
      ...
      for(const [i, liv] of [[0,1],[2,0],[4,3]]){
        ...
        items.push({ a:ca, b:cb, z:0, draw:() =>
          flatsGameCar((a,b,h) => P(ca + b, cb + a, h), FLATS_CAR_COLORS[liv]) });
      }
    }
...
},
{
  ...
  draw(p){
    /* ============ THE FLATS PASS (2026-09-17) ============
       Sandpiper Cottages. Sandstone goes whitewash, brown goes
       sea blue, the red roof goes grey slate.
       Geometry untouched: every edit is a colour. */
