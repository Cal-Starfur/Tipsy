...
| `prop.palmDwarf` | Bush-type dwarf palm (PALM_DWARF preset: h55 · f8 · droop 0.3 · wind 0.3). Same renderer as `prop.palm`. Placed in routes: ~25% of building-side plantings roll dwarf; SOFT hazard (unlike the solid tall palm) — sev 6 impact + speed ×0.5 bush drag, drive-through-able | `PALM_DWARF` const (same palette) |
...
| `prop.hydrant` / `prop.hydrantBurst` | Approved hydrant-lab red: tapered barrel + shoulder + dome + nozzles (h20 · base 8 · dome 6.5 · nozzle 2.6). NOW WIRED (hydrant hit lab): curb-side lane fixtures, hard blocker (palm contract). Hit fast (≥ thresh) and the nozzle shears → `hydrantBurst`: animated water arc + flood pooling around the base (grows ~1.8s to r41). Driving the flood with speed → HYDROPLANE: eased pirouette (`HYD_SLIP`: spin × entry speed), tilt wobble, grip unwind, robot eyes go alert-orange for the slide | `HYD`: cap `0xb5342b`/`0x8a2822`, water `0xbfe6f2`; `HYD_SLIP`: thresh 0.055 · bonk 10 · spin 1.0 · grip 1.0 · pud 41.4 |
